...
CombineGRPC depends on Swift gRPC v1.0.0-alpha.17, and integrates with Apple's new Combine framework.
...
Next, download the latest version of grpc-swift with NIO support. Currently that means [Swift gRPC 1.0.0-alpha.17](https://github.com/grpc/grpc-swift/releases/tag/1.0.0-alpha.17). Unarchive the downloaded file and build the Swift gRPC plugin by running make in the root directory of the project.
...
```swift
dependencies: [
  .package(url: "https://github.com/vyshane/grpc-swift-combine.git", from: "0.14.0"),
],
```
